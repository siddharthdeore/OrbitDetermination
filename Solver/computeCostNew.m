%% compute cost from given state and observations
function [cost] = computeCostNew(state)
global terms min_in_day timezone rho_obs stationLat stationLon stationAlt xp yp dut1 lod ddpsi ddeps dat jd_obs t_obs RA_comp DEC_comp
    [~, ~, ~, satrec] = twoline2rvDouble2(state); % with bstar
